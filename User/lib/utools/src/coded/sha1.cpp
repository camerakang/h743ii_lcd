...
static void HV_SHA1Transform(
    uint32_t state[5],
    const unsigned char buffer[64]);

static void HV_SHA1Init(
    HV_SHA1_CTX *context);

static void HV_SHA1Update(
    HV_SHA1_CTX *context,
    const unsigned char *data,
    uint32_t len);

static void HV_SHA1Final(
...
{
    void sha1(unsigned char *input, uint32_t inputlen, unsigned char digest[20])
    {
        HV_SHA1_CTX ctx;
        HV_SHA1Init(&ctx);
